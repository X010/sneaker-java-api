 ...
 */
@Controller("salemancolumnapi")
@RequestMapping(value = "/saleman/column")
public class ColumnApi extends BaseController {
    ...
    public void colums(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            columns = this.columnService.getColumnByCompanyId(getLoginUser(request).getCid());
            if (columns != null) {
    ...
    public void focus(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            User user = getLoginUser(request);
            List<Column> columns = this.columnService.getAdByCompanyId(user.getCid());
            if (columns != null) {
                responseMessage.setData(columns);
            }
        } catch (SessionException e) {
