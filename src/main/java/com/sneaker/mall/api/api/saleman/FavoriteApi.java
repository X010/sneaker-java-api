 ...
 */
@Controller("salemanfav")
@RequestMapping(value = "/saleman/fav")
public class FavoriteApi extends BaseController {
    ...
    public void canFav(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            long uid = getLoginUser(request).getId(); //获取用户ID
            long lmgid = Long.valueOf(mgid);
            boolean res = this.favoriteService.canFavoriteForUser(uid, lmgid);
    ...
    public void getFav(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            long ccid = RequestUtil.getInt(request, "ccid", 0);
            long uid = getLoginUser(request).getId();
            //读取商品列表
            List<Goods> goodses = this.goodsService.getFavoriteGoodsForUser(uid, page, limit, getLoginUser(request).getCid(), ccid);
            if (goodses != null) {
                responseMessage.setData(goodses);
                this.priceService.supplementGoodsPrice(goodses, ccid);
                responseMessage.setCount(this.favoriteService.countFavoriteForUser(uid));//补充条数，给前端分页使用
            }
        } catch (SessionException se) {
            responseMessage.setStatus(306);
            responseMessage.setMessage("login fail");
    ...
    public void deleteFav(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            long lmgid = Long.valueOf(mgid);
            //根据GIDAND CID找到收藏对象
            List<Favorite> favorites = this.favoriteService.getFavoriteByUidAndGid(uid, lmgid);
            if (favorites != null) {
